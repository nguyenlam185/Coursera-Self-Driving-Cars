...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        """
        self.vars.create_var('v_previous', 0.0)
        self.vars.create_var('pid', PID(2, 0.2, 0.5))
        ...
        if self._start_control_loop:
            ...
            self.vars.pid.setpoint = v_desired
            # Change these outputs with the longitudinal controller. Note that
            # brake_output is optional and is not required to pass the
            # assignment, as the car will naturally slow down over time.
            throttle_output = self.vars.pid(v)
            brake_output    = 0
            ...
            p2 = np.array([waypoints[0][0], waypoints[0][1]])
            p1 = np.array([waypoints[-1][0], waypoints[-1][1]])
            crosstrack_error = np.cross(p2-p1, p1-current_xy) / np.linalg.norm(p2-p1)
